# SERank
This tool reads the Java source-code files of the project, generates the abstract syntax tree, and navigates it to identify each sensitive classifier (class, interface, enumeration) based on pre-defined keywords dictionary and rules. The tool quantifies the sensitivity score of classifiers (based on thier sensitive properties) then ranks them accordingly to assists developers in prioritizing security efforts.
