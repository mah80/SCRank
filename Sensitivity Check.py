...
#Take the path of the source code from the user to collect the .java files and save them in a single file
#===============================================================================
...
#Clear the files
##################################################################
filename = ["Output/Class Attributes.csv",
            "Output/Class Methods.csv",
            "Output/Method Parameters.csv",
            "Output/Method Local Variables.csv",
            'Output/Sensitive Classes.csv',
            'Output/Sensitive Local Variables-Based Methods.csv',
            'Output/Sensitive Parameters-Based Methods.csv',
            'Output/Unsorted Sensitive Classes.csv',
            'Output/Sorted Sensitive Classes.csv',
            'Output/Method Assignment Variables.csv',
            'Output/Sensitive Assignment Variables-Based Methods.csv'
            ]
...
#The nodeNameFinder function that accepts the start point and the end point of a node to slice 
...
###A query to look for all attributes in the class
###################################################### 
...
#The structure of the csv file is created as follows: Each class takes a row of the file, the first 
#cell of the row is the class name followed by its methods
#===============================================================================
...
###A query to look for all methods in the class
######################################################    
...
###Extract each method from the capture and put it in a list to save it later in the file
############################################################################################     
...
#to look for the parameters of each method and write them in a file (Output/Method Parameters.csv)
...
def methodParametersFinder(node, className, class_methods):
    ...
    with open('Output/Method Parameters.csv', 'a', newline='') as file:
        writer = csv.writer(file, dialect='excel')
...
###A query to look for all parameters of any method in the class
######################################################   
...
#to look for the types of each local variable in each method and write them in a file (Output/Method Local Variables.csv)
#The structure of the csv file is created as follows: Each class takes a row of the file, the first cell of the row is the class name,
...
def methodLocalVariablesFinder(node, className, class_methods):
    ...
    with open('Output/Method Local Variables.csv', 'a', newline='') as file:
        writer = csv.writer(file, dialect='excel')
...
##(Output/Method Assignment Variables.csv) The structure of the csv file is created as follows:
...
def methodLocalAssignmentsFinder(node, className, class_methods):
    ...
    with open('Output/Method Assignment Variables.csv', 'a', newline='') as file:
        writer = csv.writer(file, dialect='excel')
...
###A query to look for all assignments in any method of the class and extract the left identifier (with this) of the assignment
######################################################      
...
#The keywordsCheckForAttribute() function that accepts a term/attribute (which was extracted from a csv file) to look for it
# in Keywords Dictionary file. If it is existed, the function returns True. Otherwise, it returns False.
#######################################################################################  
def keywordsCheckForAttribute(term, path):
...
#The keywordsCheckForVariableAssignment() function that accepts a term (which represents a left local variable in the assignment) to look for it
# in Sensitive Classes file. If it is existed in the sensitive attributes of the same class, the function returns True. Otherwise, it returns False.
#######################################################################################  
def keywordsCheckForVariableAssignment(term, className, path):
...
def sensitiveClassCheck():
    ...
    with open ('Output/Class Attributes.csv', 'r') as CAfile, open ('Output/Sensitive Classes.csv', 'a', newline='') as SCfile:
...
#       
              ...
              if len(temp) > 1:             ###If the class has at least one attribute, then it will be sent to the keywordsCheckForAttribute() function.
                  ...
                  for i in temp[1:]:
                      flag = keywordsCheckForAttribute(i, keywordsDictionaryPath)
                      if flag:
...
#The sensitiveMethodParametersCheck() function that checks the parameter type of each method (in the Method Parameters file) whether they are sensitive (existed in  
##Sensitive Classes file). If yes, then the function writes the method name and its sensitive parameters in the Sensitive Parameters-Based Methods file.
#===============================================================================
def sensitiveMethodParametersCheck():
    
    
    with open ('Output/Method Parameters.csv', 'r') as MPfile, open ('Output/Sensitive Classes.csv', 'r') as SCfile, open ('Output/Sensitive Parameters-Based Methods.csv', 'a', newline='') as SPBMfile:
        ...
        for i in methodParametersReader:        ###Read the Method Parameters file line by line (Each line in the file 
            ...
            if len(temp) > 2:                  ###If the method has at least one parameter, then it will be sent to the keywordsCheckForAttribute() function.
...
#The sensitiveMethodLocalVariablesCheck() function that checks the local variable type of each method (in the Method Local Variables file) whether are 
...
def sensitiveMethodLocalVariablesCheck():
    ...
    with open ('Output/Method Local Variables.csv', 'r') as MLVfile, open ('Output/Sensitive Classes.csv', 'r') as SCfile, open ('Output/Sensitive Local Variables-Based Methods.csv', 'a', newline='') as SLVBMfile:
        ...
        for i in methodLocalVariablesReader:        ###Read the Method Local Variables file line by line (Each line in the file represents a method record)
            ...
            if len(temp) > 2:                       ###If the method has at least one local variable, then it will be sent to the keywordsCheckForAttribute() function.
...
def sensitiveMethodLocalAssignmentsCheck():
    ...
    with open ('Output/Method Assignment Variables.csv', 'r') as MAVfile, open ('Output/Sensitive Assignment Variables-Based Methods.csv', 'a', newline='') as SAVBMfile:
        ...
        for i in methodAssignmentVariablesReader:        ###Read the Method Local Assignment Variables file line by 
            ...
            if len(temp) > 2:                       ###If the method has at least one local assignment variable, then it will 
                                                    ##be sent to the keywordsCheckForAttribute() function.
                                                    ###Any method without local variables will not be sent.
                
                
                
                sensitive_methods.append(className)  ###Add the class name as a first element in the row
                sensitive_methods.append(temp[1])  ###Add the method name as a second element in the row
                for i in temp[2:]:
                    flag = keywordsCheckForVariableAssignment(i, className, sensitiveClassesPath)
                    if flag:
                        sensitive_methods.append(i)
...
##The searchForSensitiveMethod function that accepts the class name and opens the Sensitive Local Variables-Based Methods.csv, the 
##Sensitive Parameters-Based Methods.csv, and Sensitive Assignment Variables-Based Methods.csv files to look for the sensitive local variable,
# sensitive parameter, and sensitive assignment variable in each file. If it exists, the 
##function returns the method name saved in a list.
#===============================================================================
def searchForSensitiveMethod(className):
    ...
    SLVBMfile = 'Output/Sensitive Local Variables-Based Methods.csv'
    SPBMfile = 'Output/Sensitive Parameters-Based Methods.csv'
    SAVBMfile = 'Output/Sensitive Assignment Variables-Based Methods.csv'
    ...
    with open(SAVBMfile, 'r') as SAVBMfile:
        SAVBMfile_reader = csv.reader(SAVBMfile)
        for row in SAVBMfile_reader:
            if (row[0] == className):
                allSensitiveMethods.append(row[1])
...
def classSensitivityCount(classNames):
    ...
    with open ('Output/Sensitive Classes.csv', 'r') as SCfile, open ('Output/Class Attributes.csv', 'r') as CAfile, open('Output/Unsorted Sensitive Classes.csv', 'a', newline='') as USSCfile:
        ...
        for i in sensitiveClassesReader:                    ##Take every sensitive class from the file
            ...
            sensitiveMethod = removeDuplication(sensitiveMethod)        ##Same method may be repeated in the list (it could be sensitive based on local variable and parameter at the same time)
...
def methodCounter(className):
    ...
    counter = 0
    with open('Output/Class Methods.csv', 'r') as CMfile:
         classMethodsReader = csv.reader(CMfile)
         #print("This is the class name: ", className)
...
##The sortSensitiveClasses() function that opens the Unsorted Sensitive Classes.csv file to sort all the sensitive classes based on the
##sensitivity level value and write them in a new file (Sorted Sensitive Classes.csv)  
#===============================================================================
def sortSensitiveClasses():
    
    with open('Output/Unsorted Sensitive Classes.csv', 'r') as USSCfile:
    ...
    with open('Output/Sorted Sensitive Classes.csv', 'w', newline='') as SSCfile:
...
#Find the sensitive methods based on the local assignments
####################################################################
